...
for i = 1:size(mapData,1)
    ...
    for j = 1:size(mapData,2)/2-1
        if (vx(i,j+1) == 0 && vy(i,j+1) == 0)  % Last vertex in the row-- close the loop.
            obs = line([vx(i,j) vx(i,1)],[vy(i,j) vy(i,1)],'LineWidth',3,'Color',[0 0 1 0.5]);
            obstacleVerts{i}(j,:) = [vx(i,j) vy(i,j)];
            j = j-1;
            break
        else
            line([vx(i,j) vx(i,j+1)],[vy(i,j) vy(i,j+1)],'LineWidth',3,'Color',[0 0 1 0.5])
            obstacleVerts{i}(j,:) = [vx(i,j) vy(i,j)];
        end
    end
    ...
    if j == size(mapData,2)/2-1
        line([vx(i,j+1) vx(i,1)],[vy(i,j+1) vy(i,1)],'LineWidth',3,'Color',[0 0 1 0.5])
...
% Draw the boundary
bound = line([vxMin vxMax],[vyMin vyMin],'LineWidth',3,'Color',[0 0 0]);
...
legend([bound,obs],'boundaries','obstacles')
